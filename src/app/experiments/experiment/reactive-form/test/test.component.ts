...
export class TestComponent {
  ...
  constructor() {
    ...
    this.mockService.getTrainingAndType()
      .pipe(takeUntilDestroyed())
      .subscribe(trainings => {
        this.trainings = trainings
      })

    this.mockService.getWorkoutFeatures()
      .pipe(takeUntilDestroyed())
      .subscribe(workoutFeatures => {
        this.physicalFeatures = workoutFeatures
        for (const feature of workoutFeatures) {
          this.workoutForm.controls.physicalFeatures
            .addControl(feature.code, new FormControl(feature.value))
        }
      })

    this.mockService.getTrainers()
      .pipe(takeUntilDestroyed())
      .subscribe(trainers => {
        this.trainers = trainers
      })

    this.mockService.getTrainingTime()
      .pipe(takeUntilDestroyed())
  ...
  get error() {
    if (this.isInvalid(this.workoutForm.controls.firstName) ||
    ...
    if (this.workoutForm.controls.trainingSession.touched &&
      this.workoutForm.controls.trainingSession.invalid)
  ...
  phoneInput(event: any) {
    ...
    if (input.length > 1) input = `+7 (${input.slice(1, 4)}) ${input.slice(4, 7)}-${input.slice(7, 9)}-${input.slice(9, 11)}`
